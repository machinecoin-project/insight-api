...
CurrencyController.prototype._setApi = function() {
  ...
  this._apiLabel = 'coinmarketcap';
  this._api = this._coinmarketcap;
};
...
CurrencyController.prototype._coinmarketcap = function(res) {

  var self = this;

  request('https://api.coinmarketcap.com/v2/ticker/812', function(err, response, body) {
    ...
    if (!err && response.statusCode === 200) {
      self.rate = parseFloat(JSON.parse(body).data.quotes.USD.price);
    }

    res.jsonp({
      status: 200,
      data: {
        coinmarketcap: self.rate
      }
    });
